...
>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
      <div
        class="relative flex justify-center w-full max-w-sm mx-auto lg:max-w-none lg:mx-0"
      >
        <div
      ...
      >
        ...
        <h2
          class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-secondary leading-tight"
        >
          {instructor.title}
          {instructor.firstName}
        </h2>
        <h3
          class="text-2xl sm:text-3xl md:text-4xl font-bold text-primary mt-1"
        >
          {instructor.lastName}
        </h3>

        <p
          class="mt-4 sm:mt-6 text-body-text leading-relaxed max-w-lg text-sm sm:text-base"
        >
          {instructor.bio} Holds the rank of
          <strong class="text-secondary font-bold">{instructor.rank}</strong>
          in ITF Taekwondo.
        </p>

        <div
          class="mt-6 sm:mt-8 relative p-4 sm:p-5 bg-white rounded-r-xl border-l-4 border-primary shadow-sm w-full max-w-lg"
        >
          <svg
            class="absolute top-2 left-2 w-6 h-6 text-primary/20"
            fill="currentColor"
            viewBox="0 0 24 24"
          >
            <path
          ...
          <p
            class="text-body-text leading-relaxed italic text-sm sm:text-base relative z-10 ml-4"
          >
            "{instructor.quote}"
          </p>
        </div>
        ...
        <div
          class="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6 mt-10 w-full max-w-lg"
        >
          {#each skills as skill}
            <div class="flex items-center gap-4 bg-white/50 p-2 rounded-xl">
              ...
              <span
                class="text-xs sm:text-sm font-bold text-secondary uppercase tracking-wider leading-tight text-left"
              >
                {skill.label}
              </span>
            </div>
