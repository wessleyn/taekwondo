<script lang="ts">
  ...
  async function addStudent(event: Event) {
    ...
    const { error } = await supabase
      .from("students")
      .insert([{ name, belt_rank, belt_order, date_joined, image_url }]);
...
<div class="max-w-2xl mx-auto py-20 px-4">
  <h1 class="text-3xl font-bold font-heading text-secondary mb-8">
    Add New Student
  </h1>

  <form
    onsubmit={addStudent}
    class="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 flex flex-col gap-6"
  >
    <div
      class="flex flex-col items-center sm:flex-row gap-6 p-4 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50/50"
    >
      <div
        class="w-24 h-24 shrink-0 rounded-full border-2 border-gray-200 bg-white overflow-hidden shadow-sm flex items-center justify-center"
      >
        {#if avatarPreview}
          <img
            src={avatarPreview}
      ...
      <div class="flex-1 w-full text-center sm:text-left">
        <label class="block text-sm font-bold text-secondary mb-2" for="avatar"
          >Profile Photo</label
        >
        <input
          type="file"
    ...
    <div>
      <label class="block text-sm font-bold text-secondary mb-2" for="name"
        >Student Name</label
      >
      <input
        type="text"
    ...
    <div>
      <label class="block text-sm font-bold text-secondary mb-2" for="belt"
        >Belt Rank</label
      >
      <select
        id="belt"
    ...
    <div>
      <label class="block text-sm font-bold text-secondary mb-2" for="date"
        >Date Joined</label
      >
      <input
        type="date"
